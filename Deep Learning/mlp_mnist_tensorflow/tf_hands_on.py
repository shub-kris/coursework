...
x = tf.random.normal([])
y = tf.random.normal([])
r = tf.case([(x < y, lambda: tf.divide(x,y)),(x > y, lambda: tf.multiply(x,y))], default= lambda: tf.add(tf.pow(x,2) , tf.pow(y,2)))
tf.print('\npart (b): ',[x,y,r])
###############################################################################
...
###############################################################################
x = tf.constant([[1, -4, -1], [0, 3, 2]])
y = tf.ones_like(x)
result = tf.equal(tf.abs(x),y)
tf.print('\npart (c): ',result)
###############################################################################
...
result = tf.cond(avg < 0, lambda: tf.reduce_mean(tf.math.squared_difference(x,y)), lambda: tf.reduce_sum(tf.abs(x-y)))
tf.print('\npart (e): ',[avg,result])
###############################################################################
...
result = tf.cond(tf.reduce_sum(x) > tf.reduce_sum(y), lambda: tf.concat([x,y],0), lambda:tf.concat([x,y],1))
tf.print('\npart (f): ', [result])
###############################################################################
