...
def gradient_descent(
    weights: [np.array], lr: float, steps: int, inputs: np.array, targets: np.array
) -> np.array:
    """
    Args:
...
def main():
    ...
    weights_trained = gradient_descent(
        weights=weights, lr=0.1, steps=10000, inputs=train_inputs, targets=train_targets
    )
    ...
    print(
        "Test accuracy: %.2f" % (accuracy(test_inputs, test_targets, weights_trained))
    )
