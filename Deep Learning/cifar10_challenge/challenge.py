...
def main():
    ...
    # load data
    eval_data_size = 5000
    (x_train, y_train), (x_test) = np.load(
        args.data_dir + "/WS1920_challenge_data_set.npy", allow_pickle=True
    )
    ...
    test_set = (
        tf.data.Dataset.from_tensor_slices(x_test).batch(args.batch_size).prefetch(2)
    )
    ...
    for e in range(args.epochs):
        ...
        tf.print(
            "epoch {0:d} \ntrain_loss: {1:2.5f} \ntrain_accuracy: {2:2.5f}".format(
                e, train_loss.result(), train_accuracy.result()
            ),
            output_stream=sys.stdout,
        )
        tf.print(
            "eval_loss: {0:2.5f} \neval_accuracy: {1:2.5f}".format(
