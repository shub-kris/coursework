...
class ClassificationNetwork(torch.nn.Module):
    def __init__(self):
        ...
        self.classes = [[-1, 0., 0. ],      # Steer Left
                        [-1., 0.5, 0.],
                        [-1., 0., 0.8],     # Steer left with brakes
                        [1, 0., 0.],        #Steer right
                        [1., 0.5, 0.],
                        [1., 0., 0.8],      #Steer right with brakes
                        [0., 0., 0.],       #Nothing
                        [0., 0.5, 0.],      #Accelerate
                        [0., 0., 0.8]]      #Brakes
    ...
    def forward(self, observation):
        ...
        fused_features = torch.cat((features_1d,speed,abs_sensors,steering,gyroscope),1)
    ...
    def actions_to_classes(self, actions):
        ...
        return [torch.Tensor([int(torch.prod(action == this_class))  # calculate the product of the elements in boolean tensor and converts them to a list 
    ...
    def scores_to_action(self, scores):
        ...
        """
        x,class_number = torch.max(scores[0], dim = 0)  #Why scores[0] only?? # Finds the max from the 9X1 column vector
        steer, gas , brake = self.classes[class_number] # Uses the classes you defined in init function to get back the appropriate values for steer,gas,brake
