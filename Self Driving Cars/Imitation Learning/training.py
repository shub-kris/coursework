...
def train(data_folder, trained_network_file):
    ...
    optimizer = torch.optim.Adam(
        infer_action.parameters(), lr=1e-4, weight_decay=0.0001
    )  # weight_decay
    observations, actions = load_imitations(data_folder)  # Loading data

    observations = [
        torch.Tensor(observation) for observation in observations
    ]  # Converts every observation numpy array into torch tensor and append to list
    ...
    actions = [
        torch.Tensor(action) for action in actions
    ]  # Converts every action numpy array into torch tensor

    batches = [
        batch for batch in zip(observations, infer_action.actions_to_classes(actions))
    ]  # Iterates over (observation, classes from classification network) (List of Tuples)
    ...
    for epoch in range(nr_epochs):
        ...
        for batch_idx, batch in enumerate(batches):  # batch_ix gives only index value
            batch_in.append(
                batch[0].to(gpu)
            )  # sends tensor to GPU the observation tensor
            batch_gt.append(batch[1].to(gpu))  # the inferred classes tensor to GPU

            if (batch_idx + 1) % batch_size == 0 or batch_idx == len(
                batches
            ) - 1:  # When everything has been appended into one list
                batch_in = torch.reshape(
                    torch.cat(batch_in, dim=0), (-1, 96, 96, 3)
                )  # stacks / append all rows into one and then reshapes
                batch_gt = torch.reshape(
                    torch.cat(batch_gt, dim=0), (-1, number_of_classes)
                )
...
def cross_entropy_loss(batch_out, batch_gt):
    ...
    loss = batch_gt * torch.log(batch_out + epsilon) + (1 - batch_gt) * torch.log(
        1 - batch_out + epsilon
    )
    loss = (
        torch.Tensor([0.25, 0.6, 0.25, 0.25, 0.45, 0.15, 0.1, 0.25, 0.1]).to(
            torch.device("cuda")
        )
        * loss
    )
